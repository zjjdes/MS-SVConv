...
def remove_ground(pcd):
    """Remove ground points from the point cloud. Optimised for Velodyne HDL-32E
    ...
    # Reorder points
    total_pts = horizon_scan * ground_scan_ind
...
def main(velo_dir, out_dir, model_file, pos_file, map_file, ref_file, rov_file, max_ref, max_trials, min_corres, voxel_size, num_kpts, non_rover, non_ground, downsample):
    ...
    for i in range(dists.shape[0]):
        ...
        while not found and j <= max_ref:
            ...
            for k in range(max_trials):
                # reg_result = fast_registration(data_s.pos[rand_s], data_t.pos[rand_t], feat_s[rand_s], feat_t[rand_t], 0.1)
                reg_result = ransac(data_s.pos[rand_s], data_t.pos[rand_t], feat_s[rand_s], feat_t[rand_t], 0.1)
                # reg_result = ransac(data_s.pos[sel_s], data_t.pos[sel_t], feat_s[sel_s], feat_t[sel_t], 0.1)

                if np.asarray(reg_result.correspondence_set).shape[0] >= min_corres:
            ...
            if inlier_rmse == 0.0 and j >= max_ref:
                ...
                j += 1
                print(' - ' + str(np.asarray(reg_result.correspondence_set).shape[0]) + ' correspondences, ' + rov_filename + ' to be reprocessed with the next nearest reference scan.')
            else:
                ...
                # kpts_s = np.asarray(data_s.pos[sel_s][corres[:, 0], :]) # TODO: apply georeferencing here?
                # kpts_t = np.asarray(data_t.pos[sel_t][corres[:, 1], :])

                print(' - Registration successful after ' + str(k + 1) + ' trials with reference scan #' + str(j + 1) + '. RMSE=' + str(inlier_rmse) + ', matched ' + str(corres.shape[0]) + ' keypoints.')
...
if __name__ == '__main__':
    ...
    out_dir = join(base_dir, 'keypoint_matching_output_ETH_3000_0.04_5_NR_test3')
    ...
    max_ref = 3 # maximum number of reference scans to test
    max_trials = 5 # maximum number of trials for RANSAC
    min_corres = 5 # minimum number of correspondence to accept transformation
    voxel_size = 0.04
    num_kpts = 3000
    ...
    main(velo_dir, out_dir, model_file, pos_file, map_file, ref_file, rov_file, max_ref, max_trials, min_corres, voxel_size, num_kpts, non_rover, non_ground, downsample)
